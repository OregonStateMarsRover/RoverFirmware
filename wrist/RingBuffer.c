...
void RingBufferInit(RingBuffer * buf, unsigned short capacity) {
	buf->data = malloc(sizeof(TYPE)*capacity);
...
unsigned short RingBufferAdd(RingBuffer * buf, TYPE * data, 
...
bool RingBufferAddByte(RingBuffer * buf, TYPE data) {
	cli();
	bool rval;
	if( buf->size < buf->cap ) {
...
TYPE RingBufferGetByte(RingBuffer * buf) {
	cli();
	TYPE rval;
	if (buf->size) {
		TYPE data = buf->data[buf->start];
...
unsigned short RingBufferGetData(RingBuffer * buf, TYPE * dest_buf, unsigned short bytes) {
	cli();
